...
def _run_refresh(fast: bool, *, include_macro: bool = True) -> None:
    _set_state(status="running", started_at=datetime.utcnow().isoformat() + "Z", finished_at=None, error=None)
    try:
        ok = try_run_refresh(refresh_market_data, fast=fast, include_macro=include_macro)
        if not ok:
            _set_state(
                status="failed",
                finished_at=datetime.utcnow().isoformat() + "Z",
                error="已有刷新任务执行中（可能与定时任务重叠），请稍后重试",
            )
            return
...
def trigger_refresh_background(fast: bool = True, *, include_macro: bool = True) -> tuple[bool, str]:
    ...
    with _lock:
        ...
        _refresh_thread = threading.Thread(
            target=_run_refresh, args=(fast,), kwargs={"include_macro": include_macro}, daemon=True
        )
